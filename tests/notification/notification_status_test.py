...
def test_notification_status_delivered(test_client):
    ...
    assert len(notifications) == 4
    assert notifications[2] ["user_id"] == 1
    assert notifications[2] ["type"] == NotificationType.ORDER_DELIVERED.value
    assert notifications[2] ["is_read"] == False
    assert notifications[2] ["message"] == "Your order status is now delivered"
    assert notifications[3] ["user_id"] == 2
    assert notifications[3] ["type"] == NotificationType.ORDER_DELIVERED.value
    assert notifications[3] ["is_read"] == False
    assert notifications[3] ["message"] == "Order #1 has been delivered"
