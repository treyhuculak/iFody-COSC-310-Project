...
def test_admin_can_delete_order(test_client):
    ...
    # Delete order as an admin (User 5 is an admin)
    delete_response = test_client.delete(f"/orders/{order_id}", headers={"X-User-Id": "5"})
    assert delete_response.status_code == 200

    # After deletion, trying to get the order as an admin should return a 404
    get_response = test_client.get(f"/orders/{order_id}", headers={"X-User-Id": "5"})
    assert get_response.status_code == 404


def test_non_admin_non_owner_cannot_delete_order(test_client):
    response = test_client.post("/orders/", json=new_order) # Customer with id 1 creates an order
    assert response.status_code == 200

    order_id = response.json()["id"] # Get the id of the newly created order

    # Try to delete order with a different customer who is niether the owner nor an admin(user 3 is a different customer, not the owner of the order)
...
def test_admin_can_add_order_item_to_order(test_client):
    # First add an order to ensure there is something to add an order item
    response = test_client.post("/orders/", json=new_order)
    assert response.status_code == 200
    order_id = response.json()["id"]

    # Now try to add an order item to that order as an admin (user 5 is an admin)
    add_response = test_client.post(f"/orders/{order_id}/items", params={"quantity": 2}, json=menu_item, headers={"X-User-Id": "5"})
    ...
    # Check if the order item was added correctly
...
def test_non_admin_non_owner_cannot_add_order_item_to_order(test_client):
    # First add an order as user 1 to ensure there is something to add an order item to
    response = test_client.post("/orders/", json=new_order)
    assert response.status_code == 200
    order_id = response.json()["id"]

    # Now try to add an order item to that order with a different customer (not the owner nor the admin)
...
def test_admin_can_delete_order_item_from_order(test_client):
    ...
    # Now try to delete that order item as an admin (user 5 is an admin)
    delete_response = test_client.delete(f"/orders/{order_id}/items/{item_id}", headers={"X-User-Id": "5"})
    assert delete_response.status_code == 200
...
def test_non_admin_non_owner_cannot_delete_order_item_from_order(test_client):
    # First add an order and an order item as user 1to ensure there is something to delete
    response = test_client.post("/orders/", json=new_order)
    assert response.status_code == 200
    order_id = response.json()["id"]
    ...
    # Now try to delete that order item with a different customer (not the owner nor the admin)
...
def test_admin_can_delete_restaurant(test_client):
    ...
    # Delete the restaurant as an admin (user 5 is an admin)
    delete_response = test_client.delete(f"/restaurants/{restaurant_id}", headers={"X-User-Id": "5"})
    assert delete_response.status_code == 200
...
def test_non_admin_non_owner_cannot_delete_restaurant(test_client):
    # First add a restaurant with user id 2 as owner
    response = test_client.post("/restaurants/", json=new_restaurant, headers={"X-User-Id": "2"})
    assert response.status_code == 200
    restaurant_id = response.json()["id"]

    # Delete the restaurant as the non owner and non admin (user 4 is a different restaurant owner)
...
def test_admin_can_add_menu_item_to_restaurant(test_client):
    # First add a restaurant with user id 2 as owner
    response = test_client.post("/restaurants/", json=new_restaurant, headers={"X-User-Id": "2"})
    assert response.status_code == 200
    restaurant_id = response.json()["id"]

    # Now try to add a menu item as user id 5 (the admin)
    add_response = test_client.post(f"/restaurants/{restaurant_id}/menu", json=new_menu_item, headers={"X-User-Id": "5"})

    assert add_response.status_code == 200
    data = add_response.json()

    # Check if the menu item was added correctly
...
def test_non_admin_non_owner_cannot_add_menu_item_to_restaurant(test_client):
    # First add a restaurant with user id 2 as owner
    response = test_client.post("/restaurants/", json=new_restaurant, headers={"X-User-Id": "2"})
    ...
    # Now try to add a menu item as user id 4 (not the owner nor the admin)
    add_response = test_client.post(f"/restaurants/{restaurant_id}/menu", json=new_menu_item, headers={"X-User-Id": "4"})
    
    assert add_response.status_code == 403  # Forbidden - non-owner nor admin trying to add menu item
...
def test_admin_can_delete_menu_item_from_restaurant(test_client):
    ...
    # Now try to delete that menu item as an admin (user 5 is an admin)
    delete_response = test_client.delete(f"/restaurants/{restaurant_id}/menu/{menu_item_id}", headers={"X-User-Id": "5"})
    assert delete_response.status_code == 200
...
def test_non_admin_non_owner_cannot_delete_menu_item_from_restaurant(test_client):
    # First add a restaurant and a menu item as user id 2 (the owner)
    response = test_client.post("/restaurants/", json=new_restaurant, headers={"X-User-Id": "2"})
    assert response.status_code == 200
    restaurant_id = response.json()["id"]
    ...
    # Now try to delete that menu item as the non owner and non admin (user 4 is a different restaurant owner)
    delete_response = test_client.delete(f"/restaurants/{restaurant_id}/menu/{menu_item_id}", headers={"X-User-Id": "4"})
    assert delete_response.status_code == 403

    # After deletion attempt, trying to get the menu item should be the same
